...
The process of crafting prompts to get the right output from a model is called **prompt engineering**. You can improve output by giving the model precise instructions, examples, and necessary context information not included in the model's training data. You can also tell the model how to prioritize different kinds of inputs using message roles and the instruction-following hierarchy.
